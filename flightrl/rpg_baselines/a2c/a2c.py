...
class A2C(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, kl_coef = 0.1, learning_rate=2.5e-4, vf_coef=0.5,
    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            assert issubclass(self.policy, ActorCriticPolicy), "Error: the input policy for the A2C model must be " \
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.pg_loss = -tf.reduce_mean(self.advs_ph * ratio)

                    self.approxkl = .5 * tf.reduce_mean(tf.square(neglogpac - self.old_neglog_pac_ph))

                    #self.kl_loss = tf.maximum(tf.reduce_mean(self.approxkl - 0.02 ), 0.0 )
                    loss = self.pg_loss - self.entropy * self.ent_coef + self.vf_loss * self.vf_coef #+ self.kl_loss * self.kl_coef
    ...
    def learn(self, total_timesteps, log_dir, logger,
              callback=None, log_interval=1, tb_log_name="A2C",
