...
class PPO2(ActorCriticRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    # vf_loss是用于优化值函数（V(s)）的损失函数
                    vf_losses1 = tf.square(vpred - self.rewards_ph)
                    vf_losses2 = tf.square(vpred_clipped - self.rewards_ph)
                    self.vf_loss = .5 * tf.reduce_mean(tf.maximum(vf_losses1, vf_losses2))

                    # pg_loss是用于优化策略函数（π(a|s)）的损失函数
                    # self.old_neglog_pac_ph 是旧策略下采取动作的负对数概率，
                    # 而 neglogpac 是新策略下的相应值。ratio 表示这两个概率之间的比值的指数，它是PPO目标函数中的关键部分。
                    # neglogpac 是新策略下的负对数概率-log(π(a|s))，而 self.old_neglog_pac_ph 是旧策略下的负对数概率。
                    ratio = tf.exp(self.old_neglog_pac_ph - neglogpac)

                    pg_losses = -self.advs_ph * ratio
                    pg_losses2 = -self.advs_ph * tf.clip_by_value(ratio, 1.0 - self.clip_range_ph, 1.0 +
                        self.clip_range_ph)
                    # 计算最终的策略梯度损失。它取 pg_losses 和 pg_losses2 中的较大值的均值
                    self.pg_loss = tf.reduce_mean(tf.maximum(pg_losses, pg_losses2))
                    # 计算新旧策略间的近似Kullback-Leibler散度。这个值用于监控策略更新的大小，并帮助调整学习率。
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs, update,
        ...
        if states is None:
            # 非循环策略情况 (states is None): 当不使用循环策略时，update_fac 计算为总批次数 (self.n_batch)
            # 除以每次更新的小批次数 (self.nminibatches) 和优化时期数 (self.noptepochs)。
            # 这里的+ 1可能是为了确保至少有一次更新，尽管这样的加法在整数除法中通常不影响结果。
            # 这个计算反映了每次策略更新迭代中处理的数据批次数量
            update_fac = self.n_batch // self.nminibatches // self.noptepochs + 1
        else:
            # 循环策略情况 (states is not None): 当使用循环策略时，
            # update_fac 的计算还额外除以每个小批次中的步数 (self.n_steps)。
            # 这是因为循环策略通常涉及到在时间序列上的操作，每个小批次中的步数对于整个训练过程的划分尤为重要。
            # 这种情况下，update_fac 反映了在每个优化周期中，根据时间步骤划分的数据批次数量
            update_fac = self.n_batch // self.nminibatches // self.noptepochs // self.n_steps + 1

        if writer is not None:
            # 检查writer对象是否存在：如果writer不为None，则表明需要记录训练过程的信息。
