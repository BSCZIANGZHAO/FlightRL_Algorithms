...
class ActorCriticPolicy(BasePolicy):
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            self._tanh_action = tf.tanh(self._action)
            self._deterministic_action = self.proba_distribution.mode()
            self._tanh_deterministic_action = tf.tanh(self._deterministic_action)
            ...
            if isinstance(self.proba_distribution, CategoricalProbabilityDistribution):
                ...
                # changed by Yunlong.. add tf.nn.tanh as the end of the last layer.
    ...
    def tanh_neglogp(self):
        # tanh_neglogp = tf.reduce_sum(tf.log( 1- self.tanh_action**2 + 1e-6), axis=1) - self._neglogp
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        # Changed by Yunlong 
        if deterministic:
            action, value, neglogp = self.sess.run([self.tanh_deterministic_action, self.value_flat, self.neglogp],
                                                   {self.obs_ph: obs})
            return action, value, self.initial_state, neglogp                                              
        else:
            tanh_action, action, value, neglogp = self.sess.run([self.tanh_action, self.action, 
                                                  self.value_flat, self.tanh_neglogp], {self.obs_ph: obs})
            # action, value, neglogp = self.sess.run([self.tanh_action, 
            #                                       self.value_flat, self.neglogp], {self.obs_ph: obs})
