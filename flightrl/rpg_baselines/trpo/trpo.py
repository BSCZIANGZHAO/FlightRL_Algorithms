...
# from rpg_baselines.common.policies import ActorCriticPolicy
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    vferr = tf.reduce_mean(tf.square(self.policy_pi.value_flat - self.ret))
                    ...
                    self.compute_fvp = tf_util.function([flat_tangent, observation, old_policy.obs_ph, action, atarg],
                                                        fvp)
                    self.compute_vflossandgrad = tf_util.function([observation, old_policy.obs_ph, self.ret],
                ...
                with tf.variable_scope("Adam", reuse=False):
                    self.vfadam = tf.train.AdamOptimizer(learning_rate=self.vf_stepsize)

                vf_gradients = tf.gradients(vferr, vf_var_list)
                self._vf_train = self.vfadam.apply_gradients(zip(vf_gradients, vf_var_list))
                ...
                self.compute_lossandgrad = \
                    tf_util.function([observation, old_policy.obs_ph, action, atarg, self.ret],
                                     [self.summary, tf_util.flatgrad(optimgain, var_list)] + losses)
                tf.global_variables_initializer().run(session=self.sess)
    def _initialize_dataloader(self):
        """Initialize dataloader."""
        batchsize = self.timesteps_per_batch
                ...
                while True:
                    ...
                    for k in range(1):
                        ...
                        with self.timed("computegrad"):
                            steps = self.num_timesteps + (k + 1) * (seg["total_timestep"])
                        ...
                        with self.timed("vf"):
                            for _ in range(self.vf_iters):
                                ...
                                for (mbob, mbret) in dataset.iterbatches((seg["observations"], seg["tdlamret"]),
                                    ...
                                    self.sess.run(self._vf_train,
                                                  feed_dict={self.policy_pi.obs_ph: mbob, self.ret: mbret})
