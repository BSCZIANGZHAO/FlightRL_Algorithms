...
class FlightEnvVec2(VecEnv):
    ...
    def __init__(self, impl):
        ...
        #     high=np.ones(self.num_acts) * 1.,
        #     dtype=np.float32) 
        self.per_dim = 41 # 每个维度离散化为10份
        all_actions = self.per_dim ** self.num_acts
        self._action_space = spaces.Discrete(all_actions)
        self._observation = np.zeros([self.num_envs, self.num_obs],
                                     dtype=np.float32)
        self._reward = np.zeros(self.num_envs, dtype=np.float32)
        self._done = np.zeros((self.num_envs), dtype=np.bool_)
    ...
    def step(self, action):
        decode_action = self.decode_action(action)
        self.wrapper.step(decode_action, self._observation,
                          self._reward, self._done, self._extraInfo)
    ...
    def decode_action(self, action):
        ...
        for i in range(self.num_envs):
            decode_act = []
            action_index = action[i].copy()
            for _ in range(self.num_acts):
                new_action_index = action_index % self.per_dim
                mapped_action = -1 + 2.0 * new_action_index / (self.per_dim - 1)
                decode_act.append(mapped_action)
                action_index = action_index // self.per_dim
            actions.append(decode_act)
        return np.asarray(actions, dtype=np.float32)
